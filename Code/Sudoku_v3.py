...
# ── Nama file model pkl ───────────────────────────────────────────────────────
PKL_KNN   = "KNN.pkl"    # KNeighborsClassifier + StandardScaler
PKL_LR    = "LR.pkl"     # LinearRegression (prediksi skor)
PKL_ISO   = "ISO.pkl"    # IsolationForest + StandardScaler (deteksi anomali)
PKL_RFC   = "RFC.pkl"    # RandomForestClassifier + StandardScaler (rekomendasi difficulty)
PKL_MULTI = "Multi.pkl"  # MultiOutputRegressor(RandomForestRegressor) + StandardScaler
...
# PKL MODEL HELPERS
# =====================================================
_PKL_DIR = os.path.dirname(os.path.abspath(__file__))

def _pkl_path(name: str) -> str:
    """Kembalikan path lengkap untuk file <name>.pkl di direktori yang sama."""
...
class PlayerMLEngine:
    ...
    def _train_models(self):
        ...
        if n >= 3:
            cached_lr = _load_pkl(PKL_LR) if n == 3 else None
            if cached_lr is not None and n == 3:
                self._lr = cached_lr["model"]
            else:
                try:
                    X_lr, y_lr = [], []
        ...
        if n >= 5:
            cached_iso = _load_pkl(PKL_ISO) if n == 5 else None
            if cached_iso is not None and n == 5:
                ...
                try:
                    ...
                    _save_pkl(PKL_ISO, {"model": iso, "scaler": scaler_iso})
                except Exception:
                    self._iso        = None
                    self._iso_scaler = None
...
def _train_ml_models(self, force=False):
    ...
    # ── Difficulty Recommender (RFC) ─────────────────────────────────
    # Load dari RFC.pkl jika ada dan tidak dipaksa retrain.
    if not force:
        cached_rfc = _load_pkl(PKL_RFC)
        if cached_rfc is not None:
            self._rec_model  = cached_rfc["model"]
            self._rec_scaler = cached_rfc["scaler"]
