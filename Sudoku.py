# IMPORTS — semua library disatukan di sini
...
def _ml_schedule_retrain(ml_instance):
    ...
    def _do():
        ...
        try:
            # 1. Retrain base models (KNN, HistGBR, IsolationForest)
            ml_instance._models_dirty = True
            ml_instance._train_models()
            # 2. Retrain RFC (difficulty) + Multioutput+HistGBR (stats) dengan data terbaru dari semua pemain
            ml_instance._ml_dirty = True
            ml_instance._train_ml_models(force=True)
            # 3. Retrain hint timer GBR dengan ritme gerakan terbaru
...
# ── PIL (Score Card PNG + Blur Overlay) ───────────────
try:
...
def _grab_blur_bg(root, radius: int = 14, darken: float = 0.38):
    """
    Ambil screenshot jendela saat ini, blur + gelap-kan, kembalikan PhotoImage.
    Dipanggil SEBELUM overlay ditampilkan agar hasilnya bersih (tanpa overlay).

    STRATEGI DPI-PROOF (mengatasi semua skenario Windows DPI):
    - Grab SELURUH layar dulu (tanpa bbox) — PIL selalu menggunakan physical
      pixels di sini, tidak peduli DPI-awareness proses.
    - Hitung scale EMPIRIS dari rasio: PIL_screen_size / tkinter_screen_size.
      Ini bekerja baik saat proses DPI-aware maupun DPI-unaware karena kita
      mengukur langsung dari data nyata, bukan dari API yang bisa berbohong.
    - Crop area jendela dengan koordinat yang sudah di-scale ke physical pixels.
    - Resize crop ke logical size agar pas tepat di tkinter canvas.
    - Return None jika PIL / ImageGrab tidak tersedia atau gagal.
    """
    if not PIL_AVAILABLE:
        return None
    try:
        root.update_idletasks()
        # Koordinat & ukuran jendela dalam LOGICAL pixels (tkinter)
        x = root.winfo_rootx()
        y = root.winfo_rooty()
        w = root.winfo_width()
        h = root.winfo_height()
        ...
        # Grab seluruh layar — PIL selalu mengembalikan physical pixels
        full = _PilGrab.grab()
        phys_w, phys_h = full.size

        # Ukuran layar logis dari tkinter
        logi_w = root.winfo_screenwidth()
        logi_h = root.winfo_screenheight()

        # Scale empiris: physical / logical
        # Jika proses DPI-unaware: Windows sudah memvirtualisasi sehingga
        #   logi_w == phys_w dan scale == 1.0 (benar, tidak ada koreksi).
        # Jika proses DPI-aware: logi_w < phys_w dan scale > 1.0 (koreksi aktif).
        sx = phys_w / logi_w if logi_w > 0 else 1.0
        sy = phys_h / logi_h if logi_h > 0 else 1.0

        # Konversi koordinat jendela dari logical ke physical untuk crop
        cx  = int(round(x * sx))
        cy  = int(round(y * sy))
        cx2 = int(round((x + w) * sx))
        cy2 = int(round((y + h) * sy))

        # Pastikan crop tidak keluar dari batas layar fisik
        ...
        # Resize ke logical size agar pas tepat dengan tkinter canvas
        if img.size != (w, h):
...
def _place_blur_canvas(parent, root, radius: int = 14, darken: float = 0.38,
                       pre_captured=None):
    """
    Pasang background blur ke `parent`.
    - `pre_captured`: PhotoImage yang sudah ditangkap sebelumnya (untuk kasus
      di mana screen sudah di-clear sebelum popup muncul, misal AchievementPopup).
    - Gunakan tk.Label sebagai display widget agar tidak ada masalah koordinat.
    - Fallback: frame gelap polos jika PIL tidak tersedia.
    """
    ...
    if photo:
        ...
        lbl._blur_photo_ref = photo          # cegah garbage-collection
        lbl.place(relx=0, rely=0, relwidth=1, relheight=1)
        return lbl
    else:
        # Fallback: frame polos semi-gelap
...
def _corner_icons_lower():
    """Turunkan icon pojok (theme+music) di bawah overlay yang sedang tampil."""
    try:
        app = _APP_INSTANCE
        if app is None:
            return
...
def _corner_icons_restore():
    """Kembalikan icon pojok ke posisi teratas setelah popup ditutup."""
    try:
        app = _APP_INSTANCE
        if app is None:
            return
...
_SFX_ERROR       = None
_SFX_WIN         = None
_SFX_ACHIEVEMENT = None   # fanfare badge unlock
_SFX_SELECT      = None   # pilih grid / difficulty — konfirmasi tegas
_SFX_HOVER       = None   # hover kartu — tick lembut
_SFX_CLICK       = None   # tombol navigasi / sekunder — klik netral
...
def _build_sfx():
    ...
    try:
        ...
        # SFX 3: fanfare menang (arpeggio C-E-G, tiap 150 ms)
        chunks = []
        ...
        # SFX 4: achievement unlock — glissando naik + shimmer (300 ms)
        # G4 → B4 → D5 → G5, tiap 60 ms, dengan overtone shimmer
        ach_chunks = []
        ...
        # Tambah shimmer note terakhir lebih panjang (120 ms)
        ...
        # SFX 5: select/confirm — double-ding naik (C5→E5, tiap 60ms, 120ms total)
        # Dipakai saat memilih grid size atau difficulty
        sel_chunks = []
        ...
        # SFX 6: hover tick — sine pendek sangat halus (1200 Hz, 28ms, volume rendah)
        # Dipakai saat kursor masuk ke atas kartu grid / difficulty
        t_hov = _np.linspace(0, 0.028, int(SR * 0.028), endpoint=False)
        env_hov = _np.exp(-t_hov * 120)
        _SFX_HOVER = _make_sound(
            _np.sin(2 * _np.pi * 1200 * t_hov) * env_hov * 0.18)
        # SFX 7: click netral — "pock" tegas untuk tombol navigasi/sekunder
        # Sine 440 Hz (A4), 55 ms, decay cepat — terdengar natural & tidak mengganggu
...
def _evaluate_achievements(username, session, all_sessions):
    """Evaluasi badge baru. Kembalikan list id yang baru diraih."""
    ...
    # CATATAN: session sudah disimpan ke JSON sebelum _evaluate_achievements
    # dipanggil, sehingga all_sessions SUDAH berisi sesi saat ini.
    # Gunakan fingerprint (bukan identitas objek "is not") agar sesi saat ini
    # benar-benar dikeluarkan dari prev_sessions.
...
# ── Nama file model pkl ───────────────────────────────────────────────────────
PKL_CLF     = "Player_Classifier.pkl"        # KNeighborsClassifier + StandardScaler  (notebook: Model_KNN)
PKL_SCORE   = "Score_Prediction.pkl"         # HistGradientBoostingRegressor, prediksi skor sesi berikutnya (notebook: Model_Score_Prediction)
PKL_ANO     = "Detect_Anomaly.pkl"           # IsolationForest + StandardScaler  (notebook: Model_Isolation_Forest)
PKL_DIFF    = "Difficulty_Recommender.pkl"   # RandomForestClassifier, rekomendasi difficulty (notebook: Model_Difficulty_Recommender)
PKL_PERFORM = "Performance_Prediction.pkl"   # MultiOutputRegressor + HistGradientBoostingRegressor + StandardScaler  (notebook: Model_Performance_Prediction)
PKL_HINT    = "Hint_Timer.pkl"               # GradientBoostingRegressor, prediksi waktu hint berikutnya (notebook: Model_Hint_Timer)
...
# ── FILE MUSIK ───────────────────────────────────────────────────────────────
MUSIC_FILE = os.path.join(_ASSETS_DIR, "music.mp3")
...
# PKL MODEL HELPERS
# =====================================================
_PKL_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Models", "Files")
# Buat folder Models otomatis jika belum ada saat pertama kali dijalankan
...
def apply_theme(name: str):
    """Update all C_* module-level color constants from the chosen theme dict."""
    ...
    global C_CROSS_ROW, C_CROSS_COL, C_CROSS_BOX, C_SELECTED, C_SAME_NUM
    ...
    C_WHITE     = t["C_WHITE"]
    C_CROSS_ROW = t["C_CROSS_ROW"]
    C_CROSS_COL = t["C_CROSS_COL"]
    C_CROSS_BOX = t["C_CROSS_BOX"]
    C_SELECTED  = t["C_SELECTED"]
    C_SAME_NUM  = t["C_SAME_NUM"]
...
def get_diff_theme(difficulty: str) -> dict:
    """Return the correct DIFF_THEMES entry for the current app theme."""
...
class PlayerMLEngine:
    ...
    def __init__(self):
        ...
        self._rfr           = None   # HistGradientBoostingRegressor — prediksi skor berikutnya (Score_Prediction.pkl)
        self._iso           = None
    ...
    def _pretrain_knn(self):
        ...
        Coba load KNN.pkl lebih dulu; train & simpan jika belum ada.
        Jika KNN.pkl ada (dari notebook), baca meta params (k, metric, weights).
        """
        try:
            ...
            if cached is not None:
                ...
                # Simpan best params dari notebook agar retrain berikutnya konsisten.
                # Notebook Model_Classify_Player menyimpan n_neighbors, metric, weights
                # di dalam key 'meta'.
                ...
                # Baca CV accuracy dari meta (notebook simpan 'cv_accuracy',
                # retrain internal simpan 'cv_f1_macro' — baca keduanya)
                _cv_acc = float(
                    meta.get("cv_f1_macro",
                             meta.get("cv_accuracy", 0.0))
                )
                # Jika belum tersimpan di meta, baca dari root level 'accuracy'
                if _cv_acc <= 0.0:
                    _cv_acc = float(cached.get("accuracy", 0.0))
                # Simpan ke meta lokal agar _train_models bisa membandingkan
                # dengan benar terlepas dari sumber PKL (notebook atau retrain)
                if "cv_f1_macro" not in meta and _cv_acc > 0.0:
                    meta["cv_f1_macro"] = _cv_acc
                return
            # Belum ada pkl → train dari data sintetis bawaan
            ...
            # Simpan PKL lengkap — X_syn/y_syn wajib ada agar _train_models
            # tidak jatuh ke fallback 20-sampel dan cv_f1_macro = 0.0 memastikan
            # model notebook (dari notebook Model_KNN.ipynb) akan menimpa ini.
            _save_pkl(PKL_CLF, {
                "model":    knn,
                "scaler":   scaler,
                "accuracy": 0.0,
        ...
        try:
            ...
            if n == 0:
                ...
                try:
                    ...
                    # Notebook Model_Classify_Player menyimpan 'cv_accuracy' di meta,
                    # sedangkan retrain internal menyimpan 'cv_f1_macro'.
                    # Baca keduanya agar kompatibel dengan kedua sumber model.
                    ...
                    if _n_actual < 20 and _syn_base is not None:
                        ...
                        # Notebook Model_Classify_Player menyimpan 'cv_accuracy' di meta,
                        # retrain internal menyimpan 'cv_f1_macro'. Baca keduanya.
                        _meta_ref_f1 = float(
                            _meta.get("cv_f1_macro",
                                      _meta.get("cv_accuracy", 0.0))
                        )
            ...
            # ── 2. HistGradientBoostingRegressor: prediksi skor sesi berikutnya ──
            # Sesuai notebook Model_Score_Prediction.ipynb:
            # - 4 fitur: [session_idx, time_per_cell, error_rate, hint_rate]
            # - TIDAK butuh StandardScaler (boosting berbasis histogram, invariant terhadap scaling)
            ...
            if n >= 3:
                ...
                # HistGBR notebook dilatih dari 300 pemain sintetis — jauh lebih representatif
                ...
                if cached_rfr is not None and n < 15:
                    ...
                    try:
                        ...
                        # ── Validasi dengan CV R² (bukan training R²) ──
                        # HistGBR tidak punya OOB score bawaan, pakai cross-val sebagai
                        # estimasi out-of-sample yang jujur.
                        ...
                        # Hanya simpan jika CV R² benar-benar lebih baik dari model lama
                        _save_new_rfr = new_oob > _old_ref_r2
                        self._rfr = rfr
                        if _save_new_rfr:
                            _save_pkl(PKL_SCORE, {
                                "model": rfr,
                                "r2":    new_oob,   # simpan CV R², bukan training R²
                                "meta": {
    ...
    def predict_next_score(self):
        """
        Prediksi skor sesi berikutnya menggunakan HistGradientBoostingRegressor.
        Return: (predicted_score: int, available: bool)
        Butuh minimal 3 sesi untuk aktif.
        HistGBR tidak butuh StandardScaler — langsung prediksi dari raw features.
        Input: [session_idx, time_per_cell, error_rate, hint_rate]
        """
        ...
        try:
            ...
            if self._rfr is None:
                # Coba load langsung dari RFR.pkl sebagai fallback
            ...
            # Gunakan time_per_cell tersimpan (total_time/empty_cells) — konsisten
            # dengan cara RFR dilatih baik di notebook maupun saat retrain di atas
            tpc_avg = sum(
                s.get("time_per_cell",
            ...
            hr_avg  = sum(s.get("hints_used", 0) / max(s.get("moves", 1), 1)
                          for s in recent) / 3
            # Asumsikan perbaikan 5% pada tpc, er, hr (learning curve)
    ...
    def detect_anomaly(self):
        ...
        Return: ("normal"|"anomaly"|"unknown", reason)
        """
        ...
        try:
            ...
            # Negasi agar konvensi konsisten dengan notebook v2:
            # score lebih besar = lebih anomali
            score     = float(-self._iso.decision_function(last_sc)[0])
            threshold = getattr(self, "_iso_threshold", 0.0)

            if score >= threshold:
                # Severity berdasarkan jarak dari threshold
...
class AttractorScreen(tk.Frame):
    IDLE_TIMEOUT  = 45     # detik idle sebelum attractor muncul
    ...
    @staticmethod
    def _tc():
        """Kembalikan dict warna sesuai tema aktif (dark / light)."""
        if _CURRENT_THEME_NAME == "dark":
            return {
                "bg":       "#060A10",
                "board_bg": "#0D1A2E",
                "box_line": "#3D85C8",   # biru medium — kontras kuat di atas dark
                "fixed":    ("#112244", "#79C0FF"),
                "ai":       ("#0C2E18", "#7EE787"),
            ...
            return {
                "bg":       C_BG,
                "board_bg": "#EFF6FF",
                "box_line": "#0969DA",   # biru GitHub — solid di atas latar terang
                "fixed":    ("#DBEAFE", "#1D4ED8"),
                "ai":       ("#DCFCE7", "#166534"),
    ...
    def _build(self):
        ...
        # ── Logo image (logo.png) dengan border adaptif tema ─────────────
        _att_logo = None
        _att_logo_path = os.path.join(_ASSETS_DIR, "logo.png")
        if os.path.exists(_att_logo_path):
            try:
                ...
                _att_h    = 42   # tinggi ikon di header live demo
        ...
        # Border warna accent agar logo kontras di dark & light theme
        _border_col = C_ACCENT
        _inner_col  = bg   # latar logo mengikuti bg header
        ...
        if _att_logo:
            ...
            _att_lbl.image = _att_logo   # cegah garbage collection
            _att_lbl.pack()
        else:
            # Fallback teks jika logo.png tidak ditemukan
    ...
    def _build_stats_panel(self, parent):
        ...
        # Stat cards — teks label tanpa emoji agar rendering stabil
        stat_items = [
        ...
        # Emoji medal peringkat — 🥇🥈🥉 dengan warna teks sesuai logam
        rank_medals = ["🥇", "🥈", "🥉"]
    ...
    #   Lv-1  blok 3×3 — Frame(bg=box_line_color, padx=2, pady=2)
    #          ditempatkan dengan padx=4, pady=4 → garis TEBAL antar kotak
    #   Lv-2  canvas sel — grid(padx=1, pady=1) di dalam blok
    #          → garis TIPIS antar sel; bg blok terlihat sebagai garis tersebut
    ...
    def _build_board(self, N, BOX):
        ...
        # Lv-1: satu Frame per blok 3×3 — bg = warna garis tebal antar kotak
        for br in range(BOX):
            for bc in range(BOX):
                blk = tk.Frame(
                    self.grid_frame,
                    bg=tc["box_line"],
                    padx=2, pady=2,   # ketebalan garis tipis antar sel di dalam blok
                )
                blk.grid(row=br, column=bc, padx=4, pady=4)  # jarak antar blok = garis tebal 3×3
                self._blocks[(br, bc)] = blk

        # Lv-2: canvas sel di dalam blok
        for r in range(N):
            for c in range(N):
                ...
                cv = tk.Canvas(
                    ...
                    bg=tc["empty"][0],   # warna latar sel kosong sesuai tema
    ...
    def _draw_cell(self, r, c, value, state="empty"):
        ...
        CELL = self.CELL_PX
        cv.delete("all")
        # Warna sesuai state dan tema aktif (dark / light)
        tc = self._tc()
        state_colors = {
            "fixed":     tc["fixed"],
            "ai":        tc["ai"],
    ...
    def _do_step(self):
        ...
        if self._hist_idx >= len(self._hist):
            # Selesai — mulai hitung mundur dari 3 ke 0 lalu puzzle baru
    ...
    def _on_keypress(self, event=None):
        """Abaikan tombol Modifier saja; selain itu dismiss."""
    ...
    def _on_motion(self, event=None):
        """Dismiss hanya jika mouse bergerak lebih dari 30 px dalam 2 detik."""
        now = time.time()
    ...
    def _dismiss(self, event=None):
        ...
        if self._blink_job:
            try:
                ...
                pass
        # Lepas binding global yang sudah dipasang oleh instance ini
        try:
            self.unbind_all("<Key>")
...
class LoginScreen(tk.Frame):
    ...
    def __init__(self, master, on_login, on_browse_players=None, on_attractor=None):
        ...
        self.on_attractor      = on_attractor   # callback saat idle timeout
        self.data              = load_data()
    ...
    def _build(self):
        ...
        # Branding — tampilkan logo.png jika ada, fallback ke ikon teks
        _app = self.master
        while _app and not isinstance(_app, tk.Tk):
            _app = getattr(_app, 'master', None)
        _logo = getattr(_app, '_logo_tk', None) if _app else None
        # ── Logo — klik 7x cepat untuk Easter Egg ─────────────────────
        self._egg_clicks  = 0
        self._egg_last_ts = 0.0
        if _logo:
            # Bungkus logo dengan border accent agar terlihat di dark & light mode
            ...
            _logo_lbl = tk.Label(_logo_inner, image=_logo, bg=C_SURFACE,
                                 cursor="hand2")
            _logo_lbl.pack()
            # Bind klik di seluruh area logo (wrap + inner + label)
        ...
        # Reset idle counter saat user mengetik
    ...
    def _logo_click(self, event=None):
        """Klik logo 7x (atau lebih) dalam 4 detik untuk membuka Easter Egg.
        Klik ke-8, ke-9, dst. dalam window yang sama tetap dianggap valid.
        """
        now = time.time()
        ...
        if self._egg_clicks >= 7:
            # Tidak reset counter — klik ke-8/9/dst. dalam window juga trigger
            _root = self.master
...
class DifficultyScreen(tk.Frame):
    ...
    def _build(self):
        ...
        # Tombol kembali di-place langsung pada hdr (bukan hdr_inner) agar
        # tidak masuk ke flow pack dan tidak menggeser judul ke bawah.
        # Judul selalu benar-benar terpusat di header pada semua skenario.
        if callable(self.on_back):
            ...
            # y=14: tepat di bawah gradient bar (8px) + sedikit padding
        ...
        # Title row — terpusat karena hdr_inner.pack() tanpa gangguan back_btn
...
class PerformanceDashboard(tk.Frame):
    ...
    def _build(self):
        ...
        # Foreground for gold buttons — dark text on bright gold (dark mode),
        # white text on deep amber-gold (light mode)
        ...
        # Tombol Simpan Kartu Skor (baris kedua)
    ...
    def _build_stat_tiles(self, parent, s):
        ...
        # Auto Fill tile hanya ditampilkan ketika sesi dimainkan di mode Hard
        ...
        # Dengan 10 item → 2 baris penuh × 5 kolom (rapi).
        # Dengan 11 item (Hard) → baris ke-3 berisi 1 tile (Auto Fill) yang
        # dibiarkan melebar penuh agar tidak ada kolom kosong menggantung.
        grid = tk.Frame(parent, bg=C_BG)
        grid.pack(fill="x", pady=(6, 16))
        cols = 5
        total_items = len(items)
        last_row_count = total_items % cols  # 0 = baris terakhir penuh
        for i, (ico, lbl, val, col) in enumerate(items):
            rr, cc = divmod(i, cols)
            # Jika hanya 1 tile di baris terakhir, tempatkan di kolom tengah (index 2)
            # agar tile berukuran sama dengan tile lain dan posisi terasa terpusat.
    ...
    def _build_behavior_card(self, parent, s, feat):
        ...
        # Auto Fill hanya relevan di Hard mode (fitur draft mode eksklusif Hard)
        ...
        # Jumlah baris grid menyesuaikan: 4 item = 2 baris, 5 item (Hard) = 3 baris
    ...
    def _build_skill_ml_card(self, parent, feat, stats, s):
        ...
        # Skill bars
        # ── Skill bars — prioritaskan ml_profile dari Multioutput Regressor ──
        # ml_profile tersedia jika sesi sudah diprediksi oleh stats model
    ...
    def _save_score_card_action(self):
        """Ambil data dari session + ML + achievement, generate PNG."""
        s          = self.session
        stats      = self.ml.get_summary()
        ...
        # Ambil riwayat sesi dan achievement dari JSON
        _data         = load_data()
        _pdata        = _data["players"].get(self.username, {})
...
class TutorialOverlay(tk.Frame):
    ...
    def __init__(self, master, n, on_close):
        # master = GameScreen (tk.Frame yang sudah di-place relwidth=1 relheight=1)
        # Warna overlay: gelap di dark mode, abu terang di light mode
        overlay_bg = "#080B10" if _CURRENT_THEME_NAME == "dark" else "#BCC8DC"
        super().__init__(master, bg=overlay_bg)
        self.on_close = on_close
        self.n = n
        # Tutup seluruh permukaan GameScreen
        self.place(relx=0, rely=0, relwidth=1, relheight=1)
        self._build()
        self.lift()
...
class GameScreen(tk.Frame):
    ...
    def __init__(self, master, username, grid_size, difficulty, on_finish,
        ...
        # Batas error — ditentukan oleh ML setelah engine siap
        self.max_errors = self._get_max_errors()
        # Tutorial pemain baru — hanya tampil jika pemain benar-benar baru:
        # belum punya sesi SAMA SEKALI dan belum pernah menyelesaikan tutorial.
        # Pemain lama (sudah ada sesi) tidak mendapat tutorial meski flag absen,
        # agar tidak mengganggu alur pameran saat ganti pemain.
        ...
        # ML Transparency Panel state
        self._ml_panel_visible  = False   # toggle on/off
        self._ml_panel_frame    = None    # overlay Frame widget
        self._ml_panel_job      = None    # after() handle untuk refresh loop
        self._ml_last_refresh   = 0.0    # timestamp refresh terakhir
        self._ml_refresh_busy   = False   # guard agar tidak double-refresh
        self._ml_panel_pos      = None    # (x, y) posisi terakhir — None = pakai default pojok kanan atas
    ...
    def _build(self):
        ...
        # ═══ SIDEBAR ═══
        _sb_w = 275   # lebar sidebar seragam untuk semua ukuran grid
        ...
        # ── Logo row dalam sidebar ────────────────────────────────────────
        _sb_logo = None
        _sb_logo_path = os.path.join(_ASSETS_DIR, "logo.png")
        if os.path.exists(_sb_logo_path):
            try:
                ...
                _sb_h    = 28   # ikon kecil di sidebar
        ...
        if _sb_logo:
            # Border accent tipis agar logo terlihat di kedua tema
            ...
            _sb_lbl.image = _sb_logo   # cegah garbage collection
            _sb_lbl.pack()
        else:
        ...
        # Baris Auto ⚡ hanya muncul di Hard mode karena fitur draft/auto eksklusif Hard
        if self.difficulty == "Hard":
        ...
        # ESC ditangani secara global oleh SudokuApp._on_esc_global
        self._idle_check()
    ...
    def _build_draft_panel(self):
        ...
        # Tips — satu baris agar lebih compact
    ...
    def _start_new_game(self):
        ...
        self.last_action   = time.time()
        self.hint_shown    = False
        self.max_errors    = self._get_max_errors()   # recalculate tiap new game
        ...
        # Tutorial overlay — hanya muncul sekali untuk pemain baru.
        # _tutorial_pending di-set True di __init__ jika player belum punya flag
        # tutorial_done. Langsung reset ke False di sini (before after()) agar
        # panggilan _start_new_game() berikutnya (new game / ganti difficulty)
        # tidak memunculkan tutorial lagi.
        if getattr(self, "_tutorial_pending", False):
            self._tutorial_pending = False
            self.after(500, self._show_tutorial)
    ...
    def _kiosk_reset(self):
        """Kiosk reset — langsung ke login tanpa dialog (F5 / Ctrl+Shift+R)."""
        self._stop_timer()
        self.master.event_generate("<<Logout>>")
    ...
    def _show_tutorial(self):
        """Tampilkan tutorial overlay untuk pemain baru."""
        try:
    ...
    def _on_tutorial_done(self):
        """Simpan flag tutorial_done setelah pemain menutup tutorial."""
        try:
            data = load_data()
    ...
    def _show_back_to_grid_confirm(self):
        """Tampilkan popup konfirmasi kembali ke pilih grid (bukan keluar program)."""
        # Jangan buat duplikat popup
        if hasattr(self, "_exit_popup") and self._exit_popup and \
                self._exit_popup.winfo_exists():
            return

        # ── Ambil blur screenshot SEBELUM overlay dibuat ──────────────────────
        try:
        ...
        # Mount overlay di ROOT WINDOW (bukan self) agar menutupi icon pojok
        # yang juga tinggal di root level. Sama dengan _show_exit_confirm_global.
        _overlay_parent = _root_ref if _root_ref else self
        overlay = tk.Frame(_overlay_parent, bg="")
        overlay.place(relx=0, rely=0, relwidth=1, relheight=1)
        overlay.lift()   # pastikan di atas semua widget root termasuk icon pojok
        self._exit_popup = overlay

        # Canvas blur sebagai background overlay
        if _root_ref:
        ...
        # ── Outer glow ring biru + Card terpusat ──────────────────────────────
        card_bg     = C_SURFACE
        card_border = C_ACCENT   # biru — bukan merah, karena ini navigasi bukan peringatan
        ...
        # Gradient accent stripe atas — biru→ungu (navigasi, bukan bahaya)
        _stripe_c1 = C_ACCENT
        _stripe_c2 = C_PURPLE
        ...
        # Icon + judul — desain navigasi, bukan peringatan bahaya
        ...
        # ── Sembunyikan tombol tema & musik saat popup tampil ─────────────────
        _corner_icons_lower()
        ...
        def _confirm_back():
            ...
            try:
                ...
                pass
            self._exit_popup = None
            # Kembali ke grid select (bukan tutup program)
            self._back_to_grid()
        ...
        # Hover efek tombol Lanjut Bermain
        ...
        # Klik di overlay (di luar card) → tutup popup
        dim.bind("<Button-1>", lambda _: _cancel())
        overlay.bind("<Button-1>", lambda e: _cancel()
                     if e.widget is overlay else None)

        # ESC lagi → batal (kembali bermain)
        # return "break" agar event tidak merambat ke bind_all(_on_esc_global)
    ...
    def _input_number(self, num):
        ...
        if not num_valid:
            self.error_count += 1
            _play_sfx(_SFX_ERROR)   # buzz error
            now  = time.time()
            ...
            _play_sfx(_SFX_CORRECT)   # klik ringan benar
    ...
    def _check_win(self):
        ...
        self.game_over = True
        self._stop_timer()
        _play_sfx(_SFX_WIN)   # fanfare kemenangan
    ...
    def _get_max_errors(self):
        ...
        if self.ml.sessions:
            try:
                ...
                if pt == "Struggling":
                    base = base + 1          # lebih toleran — beri ruang untuk belajar
                elif pt == "Expert":
                    base = max(1, base - 1)  # lebih ketat — tambah tantangan
            except Exception:
                pass
        return base

    def _check_lose(self):
        """Panggil setelah setiap error increment. Trigger kalah jika batas terlampaui."""
        if self.game_over:
            return
    ...
    def _trigger_lose(self):
        """Sequence game over kalah — animasi, simpan sesi completed=False, lanjut dashboard."""
        if self.game_over:
            return
        self.game_over = True
        self._stop_timer()
    ...
    def _flash_lose(self):
        """Animasi kilat merah — visual sinyal kalah."""
        cs      = list(self.canvases.values())
        t       = self.theme
    ...
    def _update_stat_labels(self):
        ...
        if self.error_count == 0:
            ...
            err_col = C_ERROR          # merah terang — bahaya
        elif remaining <= 2:
            err_col = C_WARN           # oranye — peringatan
        else:
            err_col = C_ERROR
    ...
    def _build_live_session(self):
        """Buat dict sesi saat ini dari state game yang sedang berjalan."""
    ...
    def _remaining_pct(self):
        """Hitung persentase sel yang masih kosong dari total empty_cells awal."""
        remaining = sum(
    ...
    def _build_ml_transparency_panel(self):
        ...
        W_PANEL = 292   # lebar panel dalam pixel

        # ── Hitung posisi spawn panel ────────────────────────────────────────────
        # 1. Posisi diingat (_ml_panel_pos) jika user pernah drag
        # 2. Default: pojok KANAN ATAS layar, tidak menimpa board
        self.update_idletasks()
        MARGIN_RIGHT = 40
        MARGIN_TOP   = 195
        ...
        # Outer frame ditempatkan di posisi yang dihitung
        pnl = tk.Frame(
            self,
            bg=bg_panel,
        ...
        # ── Drag-to-move: tahan header lalu seret, posisi disimpan ─────────────
        self._ml_drag_x = 0
        self._ml_drag_y = 0
        def _drag_start(e):
        ...
        # Body scrollable agar panel tidak melebihi tinggi layar
        ...
        body = body_inner_pad  # redirect body ke inner frame
        ...
        _sep()
        # ── 3. HistGBR Predicted Next Score ─────────────────────────
        ...
        # ── Footer: sklearn status + last update ──────────────────
        ...
        # Muat data pertama setelah panel selesai dirender
        self.after(120, self._schedule_ml_refresh)

    # ── Refresh loop ──────────────────────────────────────────────────────
    def _schedule_ml_refresh(self):
        """Jadwalkan refresh panel: jalankan komputasi ML di background thread."""
        if not self._ml_panel_visible:
            return
        if self._ml_refresh_busy:
            return   # refresh sedang berjalan, skip

        self._ml_refresh_busy = True
        self._ml_last_refresh = time.time()

        # Update tombol refresh sementara loading
        try:
        ...
        def _bg():
            result = {}
            # Setiap blok try sendiri -- partial failure tidak hapus data lain
            try:
            ...
            # Idle saat ini — diukur di sini agar tepat waktu
        ...
        def _done(result):
            ...
            try:
                ...
                pass
            # Jadwalkan refresh berikutnya setelah 8 detik
            if self._ml_panel_visible:
                try:
                    if self._ml_panel_job:
                        self.after_cancel(self._ml_panel_job)
    ...
    def _apply_ml_data(self, d):
        """Update semua widget ML panel dengan data hasil komputasi."""
        if not self._ml_panel_visible:
            return
        if not self._ml_panel_frame or not self._ml_panel_frame.winfo_exists():
            return
        # Pastikan panel terbuka kembali jika widget hilang
...
class PlayerSelectScreen(tk.Frame):
    ...
    def _build_detail_ui(self, st, username):
        ...
        # ── Empty state: pemain baru belum punya sesi ─────────────────
        if st["n_sess"] == 0:
            ...
            # Tombol langsung pilih pemain ini
            tk.Button(inner,
                      text="▶  Mulai Bermain",
                      font=("Segoe UI", 10, "bold"),
                      bg=C_ACCENT, fg=C_BG,
        ...
        # ── Action button column (far right) — packed BEFORE hero_text
        #    so it stays pinned to the right edge even when hero_text expands.
        # ─────────────────────────────────────────────────────────────────────
        # Pre-compute theme-aware button colours
        _btn_fg        = C_BG   # text on coloured button
        # Hover: lighten tc in dark mode, use a muted surface in light mode
        try:
        ...
        if is_me:
            # Already the active user — show a muted "LANJUTKAN" pill
            _cont_bg  = C_SURFACE2
            _cont_fg  = C_TEXT
            _cont_hov = C_BORDER
            _cont_btn = tk.Button(
            ...
            _cont_btn.pack()
            # "active" badge beneath the button
            tk.Label(hero_action,
                     text="● Akun Aktif",
                     font=("Segoe UI", 8, "bold"),
                     bg=C_SURFACE, fg="#7EE787").pack(pady=(5, 0))
        else:
            # Primary CTA — prominent, right-aligned, visible immediately
            _login_label = (
                f"▶  Login as @{username}"
                if not self.current_user
        ...
        # Compute badge background: dark tint in dark mode, pastel tint in light mode
        try:
            rr = int(tc[1:3], 16)
            gg = int(tc[3:5], 16)
            bb = int(tc[5:7], 16)
            if _CURRENT_THEME_NAME == "light":
                # Blend color 20% with white (80%) → pale pastel background
                rb = int(rr * 0.20 + 255 * 0.80)
                gb = int(gg * 0.20 + 255 * 0.80)
                bb2 = int(bb * 0.20 + 255 * 0.80)
            else:
                # 15% brightness of the type color → dark tint
                rb = int(rr * 0.15)
                gb = int(gg * 0.15)
                bb2 = int(bb * 0.15)
        ...
        # ── Info / Konfirmasi Section ─────────────────────────────
        # Tombol konfirmasi sudah berada di kanan atas (hero_action).
        # Bagian ini hanya menampilkan konteks tambahan — tidak menduplikasi tombol.
        if not is_me:
...
class SudokuApp:
    def __init__(self):
        ...
        # ── Logo (logo.png di direktori yang sama) ─────────────────────────
        self.logo_image  = None   # PhotoImage — disimpan agar tidak di-GC
        self.logo_small  = None   # versi kecil untuk header
        self.root._logo_tk = None  # juga ditempel ke root agar child frame bisa akses
        ...
        if os.path.exists(_logo_path):
            try:
                ...
                # Deteksi DPI scaling dari tkinter
                _dpi  = self.root.winfo_fpixels('1i')  # pixel per inch
                _scale = max(1.0, _dpi / 96.0)          # 96 DPI = baseline
                ...
                try:
                    ...
                    # Hitung faktor subsample agar lebar max ~48px
        ...
        # ── Kiosk Reset Hotkey — F5 / Ctrl+Shift+R ────────────────────
        # Langsung logout tanpa dialog: cocok untuk pergantian pemain cepat
        # di pameran. Aktif di semua layar (bukan hanya saat game berjalan).
        self.root.bind_all("<F5>",             self._kiosk_reset)
        self.root.bind_all("<Control-Shift-r>", self._kiosk_reset)
        self.root.bind_all("<Control-Shift-R>", self._kiosk_reset)

        # ── ESC Global — aktif di SEMUA layar ──────────────────────────
        # bind_all dipakai agar tidak ikut terhapus saat AchievementPopup
        # memanggil root.unbind("<Escape>") saat menutup dirinya sendiri.
        # Cooldown 150 ms di _on_esc_global mencegah double-fire saat
        # AchievementPopup bertransisi sinkron ke PerformanceDashboard.
        ...
        self._music_paused = False   # True jika musik sengaja di-pause via tombol M
        self._init_music()
        ...
        # ── Theme toggle button (sun/moon) — sudut kanan atas ────────
    ...
    def _init_music(self):
        ...
        try:
            ...
            _build_sfx()   # generate SFX programatik
        except Exception:
            return
    ...
    def _draw_music_btn(self, hover=False):
        """Gambar icon musik di canvas — note penuh = ON, redup + garis = OFF."""
        c = self._music_btn
        c.delete("all")
        is_dark = (_CURRENT_THEME_NAME == "dark")
        on = self._music_on

        # Warna berdasarkan tema dan state
        if is_dark:
            if on:
    ...
    def _update_music_hint(self):
        """Refresh icon musik setelah state berubah."""
        try:
            self._draw_music_btn()
        except Exception:
            pass
    ...
    def _raise_overlay(self):
        ...
        if getattr(self, "_corner_overlay_paused", False):
            return          # popup sedang tampil — jangan angkat icon dulu
    ...
    def _clear(self):
        # Catat waktu ganti layar — digunakan oleh _on_esc_global
        # agar tidak salah tembak saat AchievementPopup bertransisi sinkron.
    ...
    def _show_login(self):
        ...
        self.screen = LoginScreen(
            ...
            on_attractor=self._show_attractor,   # idle → demo screen
        )
        self.root.after(50, self._raise_overlay)

    def _show_attractor(self):
        """Tampilkan AttractorScreen saat LoginScreen idle >= 45 detik."""
        self._rebuild_fn = self._show_login   # kembali ke login saat dismiss
        self._clear()
    ...
    def _kiosk_reset(self, _=None):
        """
        Kiosk Reset — logout instan tanpa dialog konfirmasi.
        Dipanggil via F5, Ctrl+Shift+R, atau tombol 'Logout / Reset Sesi' di sidebar.
        Cocok untuk pergantian pemain cepat di pameran.
        """
        self._logout()
    ...
    def _show_exit_confirm_global(self):
        """
        Popup konfirmasi keluar dengan tampilan yang sama seperti GameScreen._show_exit_confirm.
        Dipakai oleh _on_esc_global saat screen aktif bukan GameScreen
        (LoginScreen, GridSizeScreen, dll.) sehingga visual tetap konsisten.
        Overlay selalu di-mount di self.root agar menutupi seluruh jendela dan
        blur photo sesuai ukuran penuh (mencegah tampilan 'membesar/geser').
        """
        # Jangan buat duplikat popup
        ...
        # Selalu mount di root — bukan di self.screen — agar:
        # 1. Overlay menutupi seluruh jendela (bukan hanya area screen saat ini)
        # 2. Blur photo (ukuran root) cocok persis dengan ukuran overlay
        parent = self.root

        # ── Ambil blur screenshot SEBELUM overlay dibuat ───────────────────────
        # Penting: capture SEBELUM overlay.place() agar screenshot tidak menangkap
        # frame kosong yang baru dibuat (yang akan membuat blur tampak salah).
        ...
        # Canvas blur sebagai background — pakai foto yang sudah di-capture sebelumnya
        dim = _place_blur_canvas(overlay, self.root, pre_captured=_blur_pre)

        # ── Outer glow ring merah + Card terpusat ─────────────────────────────
        card_bg     = C_SURFACE
        card_border = C_ERROR
        ...
        # Gradient stripe atas merah -> oranye -> kuning
        stripe_cv = tk.Canvas(card, height=6, bg=card_bg, highlightthickness=0)
        stripe_cv.pack(fill="x")
        ...
        # Klik di luar card -> tutup popup
        dim.bind("<Button-1>", lambda _: _cancel())
        overlay.bind("<Button-1>", lambda e: _cancel() if e.widget is overlay else None)

        # ESC lagi -> batal popup
        # return "break" wajib agar event tidak merambat ke bind_all(_on_esc_global)
        # yang akan langsung membuka kembali popup yang baru saja dibatalkan.
    ...
    def _on_esc_global(self, event=None):
        """
        Handler ESC global — aktif di SEMUA layar (bind_all).

        Logika navigasi per tipe screen:
          LoginScreen         → konfirmasi keluar aplikasi
          AttractorScreen     → kembali ke login
          PlayerSelectScreen  → kembali ke game (jika ada state tersimpan) atau ke login
          GridSizeScreen      → kembali ke login
          DifficultyScreen    → kembali ke pilih grid
          GameScreen          → popup konfirmasi kembali ke pilih grid (bukan keluar app)
          PerformanceDashboard→ kembali ke pilih grid
          AchievementPopup    → sudah ditangani sendiri, abaikan di sini

        Perlindungan ganda:
          1. Cooldown 150 ms mencegah double-fire saat AchievementPopup
             bertransisi sinkron ke screen berikutnya dalam satu event loop.
          2. Skip jika fokus ada di Entry widget (user sedang mengetik).
        """
        # Cooldown — layar baru saja diganti (misal oleh AchievementPopup)
        ...
        # AttractorScreen: tutup demo, kembali ke login
        elif isinstance(screen, AttractorScreen):
            self._show_login()

        # PlayerSelectScreen: kembali ke game jika ada state, atau ke login
        ...
        # AchievementPopup: sudah punya binding ESC sendiri di root — abaikan
        elif isinstance(screen, AchievementPopup):
            pass

        # LoginScreen (dan fallback): popup konfirmasi keluar bergaya sama
        else:
    ...
    def _draw_theme_btn(self, hover=False):
        """Draw a circular sun/moon toggle button on the canvas."""
        c      = self._theme_btn
        is_dark = _CURRENT_THEME_NAME == "dark"
        c.delete("all")

        # Colors per theme state
        if is_dark:
            ring_col  = "#2A3F5F"
            ring_hi   = "#3A5F8F"
            icon_col  = "#FFD700"   # sun gold
            bg_fill   = "#1A2840" if not hover else "#223050"
        else:
            ring_col  = "#C5CFE0"
            ring_hi   = "#A0B0CC"
            icon_col  = "#4A80C8"   # moon blue
            bg_fill   = "#E0E8F4" if not hover else "#CCD6EC"
        ...
        if is_dark:
            ...
            # ── Moon icon ────────────────────────────────────────
            # Crescent: big circle minus offset small circle (layered)
    ...
    def _toggle_theme(self, _=None):
        """Switch between dark and light themes and rebuild current screen."""
        ...
        # Redraw both overlay buttons with new theme colors
        self._draw_theme_btn()
        self._draw_music_btn()

        # Lift both buttons above any new screen widgets
        self.root.tk.call("raise", self._theme_btn._w)
        self.root.tk.call("raise", self._music_btn._w)
...
def _ml_session_vector(session):
    ...
    # Pakai time_per_cell yang tersimpan (total_time / empty_cells) — sesuai
    # definisi notebook Model_GBM dan Model_Multioutput_Regressor.
    # Fallback ke total_time/moves hanya jika field belum ada (sesi lama).
...
def _ml_aggregate_vector(sessions):
    if not sessions:
        return [0.0] * 8
    n = len(sessions)
    # Pakai time_per_cell (total_time/empty_cells) sesuai definisi notebook
    tpc = sum(
        s.get("time_per_cell",
...
def _ml_init(self):
    ...
    self._iso_threshold = 0.0
    self._ml_dirty      = True
    self._hint_rfr      = None   # GradientBoostingRegressor — model Hint Timer
    self._hint_scaler   = None   # scaler hint timer
    # knn best params — di-populate oleh _pretrain_knn / _train_models
    self._knn_best_k       = 3
    self._knn_best_metric  = "euclidean"
    self._knn_best_weights = "uniform"
    # Load Score_Prediction.pkl dari notebook saat init agar predict_next_score siap sejak sesi ke-3
    if SKLEARN_AVAILABLE:
...
def _train_ml_models(self, force=False):
    ...
    # Difficulty Recommender (Difficulty_Recommender.pkl dari notebook Model_Difficulty_Recommender)
    # Load dari PKL jika ada dan tidak dipaksa retrain — model yang tersimpan adalah RFC.
    if not force:
    ...
    # Hitung total sesi nyata dari semua pemain (dipakai untuk ambang proteksi)
    _real_sess_count = sum(len(s) for s in _ml_all_sessions())

    # ── PROTEKSI RFC: jika sesi nyata < 30, pakai model notebook ────────────
    # RFC notebook dilatih dari 5000 sampel dengan distribusi Beta/Gamma realistis.
    # Fallback sintetis di sini hanya Uniform — menghasilkan boundary yang terlalu rapi
    # dan training accuracy tinggi palsu yang bisa menimpa model notebook.
    _cached_gbm_pre = _load_pkl(PKL_DIFF)
    # Ambil metrik referensi dari PKL.
    # Notebook Model_Difficulty_Recommender menyimpan 'f1_score' (F1 Macro test set).
    # Retrain internal menyimpan 'f1_score' juga. Fallback ke 'accuracy' untuk
    # kompatibilitas mundur dengan format PKL lama yang mungkin menyimpan 'accuracy'.
    ...
    if _protect_gbm:
        ...
        # Input 8 fitur: [tpc, er, hr, cr, nmr, gur, avg_moves, avg_score]
        # Label: 0=Easy, 1=Normal, 2=Hard  (identik dengan notebook Model_GBM)
        X_rec, y_rec = [], []
        ...
        if len(X_rec) < 8:
            # Fallback sintetis — distribusi identik dengan notebook Model_GBM
        ...
        from sklearn.model_selection import cross_val_score, StratifiedKFold
        # RFC (Random Forest Classifier) — model resmi untuk Difficulty Recommender
        ...
        # ── Validasi dengan CV F1 Macro (bukan training accuracy) ──
        # # F1 Macro identik dengan metrik notebook — wajib lebih baik agar bisa menyimpan ulang.
        ...
        # Hanya update memori DAN simpan PKL jika model RFC baru murni lebih baik.
        # Jika tidak, tetap pakai model notebook RFC di memori agar prediksi akurat.
        ...
        if _save_new_rfc:
            ...
            _save_pkl(PKL_DIFF, {
                "model":    rec_model,
                "scaler":   rec_scaler,
                "f1_score": new_rfc_f1,   # simpan CV F1 Macro
                "meta": {
                    "n_train":    len(X_rec),
            ...
            # Model baru tidak lebih baik — reload notebook model ke memori
            if _cached_gbm_pre is not None:
                self._rec_model  = _cached_gbm_pre["model"]
                self._rec_scaler = _cached_gbm_pre["scaler"]
            else:
                self._rec_model  = rec_model   # tidak ada pilihan lain
                self._rec_scaler = rec_scaler

    # ── Stats Model (Performance_Prediction.pkl dari notebook) ─────────────
    # Input 8 fitur: [tpc, er, hr, cr, nmr, gur, avg_moves, avg_score]
    # Output 11 target: exp_tpc, exp_er, exp_hr, exp_cr, exp_nmr, exp_gur,
    ...
    # PROTEKSI: lindungi model notebook selama sesi nyata < 100
    # 11 output regresi butuh data yang sangat banyak agar retrain bermakna.
    # Notebook dilatih dari 5000 sampel dengan 5 archetype terstruktur.
    _protect_multi = (
    ...
    if _protect_multi:
        ...
        if len(X_stats) < 8:
            # Fallback sintetis — distribusi identik dengan notebook Model_Multioutput_Regressor
            X_stats, Y_stats = [], []
        ...
        # MultiOutputRegressor + HistGBR — model resmi untuk Performance Prediction
        ...
        # ── Validasi dengan CV R² per-output, rata-rata (bukan training R²) ──
        # CV R² jauh lebih konservatif — training R² MultiOutput hampir selalu
        # mendekati 1.0 karena 11 output yang sebagian besar adalah transformasi
        # deterministik dari input.
        try:
            ...
            # Fallback: pakai training R² tapi tandai sebagai tidak reliable
            new_multi_r2 = float(stats_model.score(X_stats_sc, Y_stats)) * 0.80

        # Hanya update memori DAN simpan PKL jika CV R² murni lebih tinggi
        # dari R² model notebook. Jika tidak, reload notebook model ke memori.
        ...
        if _save_new_multi:
            ...
            if _cached_multi_pre is not None:
                ...
                self._stats_model  = stats_model   # tidak ada pilihan lain
                self._stats_scaler = stats_scaler
...
def _ml_get_summary(self, session=None):
    ...
    # Ambil metadata RFR.pkl untuk ditampilkan jika tersedia
    _rfr_meta = {}
    try:
        _rfr_pkg = _load_pkl(PKL_SCORE)
        if _rfr_pkg is not None:
...
def _ml_train_hint_model(self):
    """
    Retrain HintTimer model menggunakan data sesi nyata terbaru.

    Strategi:
    - Model WAJIB sudah tersedia di PKL_HINT (dilatih dari notebook).
      Jika belum ada, fungsi ini tidak akan membuat model dari nol.
    - Retrain hanya dilakukan jika jumlah sesi nyata baru >= 10
      agar model tidak overfit pada data yang sangat sedikit.
    - Menggunakan 15 fitur (9 original + 6 engineered) sesuai notebook.
    - Menyimpan model baru ke PKL hanya jika CV MAE lebih baik dari
      model yang sudah ada.
    """
    if not SKLEARN_AVAILABLE:
        return

    # ── WAJIB: model notebook harus sudah ada ────────────────────────────────
    cached_pkg = _load_pkl(PKL_HINT)
    if cached_pkg is None:
        # Model belum dibuat dari notebook — tidak ada yang bisa di-retrain
        return

    # ── Kumpulkan sesi nyata dari semua pemain ────────────────────────────────
    all_player_sessions = _ml_all_sessions()
    flat = [s for player_sess in all_player_sessions for s in player_sess]

    # Minimal 10 sesi nyata sebelum retrain dilakukan
    if len(flat) < 10:
        # Tidak cukup data — pertahankan model notebook di memori
        if self._hint_rfr is None:
            self._hint_rfr    = cached_pkg["model"]
            self._hint_scaler = cached_pkg.get("scaler", None)
        return

    # ── Bangun feature matrix (15 fitur) dan label ───────────────────────────
    def _hint_session_to_fv(s):
        """Konversi 1 sesi → feature vector 15 dimensi."""
        ...
        # 6 fitur engineered (sesuai notebook HintTimer_RFR_Improved)
    ...
    def _hint_label(s):
        """Label idle threshold (detik) dengan log-smoothing untuk outlier."""
        moves     = max(s.get("moves", 1), 1)
        total_t   = float(s.get("total_time", 0) or 0)
        hr        = s.get("hints_used", 0) / moves
        ...
        # Log-smoothing: tarik outlier ekstrem ke arah median
        if raw > 60:
    ...
    try:
        ...
        # GradientBoostingRegressor (GBR) — model resmi untuk Hint Timer adaptif
        ...
        # ── Validasi CV MAE — hanya simpan jika lebih baik dari model lama ──
        ...
        if new_mae < _old_mae:
            ...
            # Model lama lebih baik — tetap pakai yang dari PKL
            self._hint_rfr    = cached_pkg["model"]
            self._hint_scaler = cached_pkg.get("scaler", None)
    except Exception:
        # Fallback: pastikan model notebook tetap aktif di memori
        if self._hint_rfr is None:
            self._hint_rfr    = cached_pkg["model"]
            self._hint_scaler = cached_pkg.get("scaler", None)
...
def _ml_compute_hint_threshold(self, grid_size=3, difficulty="Normal",
                                remaining_pct=1.0):
    """
    Prediksi idle threshold adaptif per-pemain menggunakan model HintTimer.

    Fitur (15 dimensi — sesuai notebook HintTimer_RFR_Improved):
      Original (9): [avg_tpc, error_rate, hint_rate, completion_rate,
                     near_miss_rate, guessing_rate, grid_n, diff_int, remaining_pct]
      Engineered (6): [log_tpc, err_x_hint, diff_x_grid,
                       hint_pressure, move_density, patience_proxy]

    Output: detik idle sebelum banner hint muncul (float, 8–120 detik).
    Jika model PKL belum ada, gunakan estimasi heuristik sebagai fallback.
    """
    ...
    # ── Fitur engineered (6) — identik dengan notebook ────────────────────────
    log_tpc        = float(np.log1p(avg_tpc))
    err_x_hint     = error_rate * hint_rate
    diff_x_grid    = diff_int * (grid_n / 81.0)
    hint_pressure  = hint_rate / (completion_rate + 0.01)
    # move_density: estimasi dari avg sesi (tidak ada konteks moves saat ini)
    ...
    fv_15 = np.array([orig + [log_tpc, err_x_hint, diff_x_grid,
                               hint_pressure, move_density, patience_proxy]],
                      dtype=float)
    fv_9  = np.array([orig], dtype=float)  # fallback jika model lama (9 fitur)

    def _try_predict(model, scaler, fv_full, fv_short):
        """Coba prediksi dengan 15 fitur, fallback ke 9 fitur untuk model lama."""
        try:
    ...
    # ── Heuristik numerik jika PKL belum ada sama sekali ─────────────────────
...
def _ml_dashboard_session(session, ml):
    """
    Buat salinan sesi untuk dashboard dengan tambahan field ml_profile.
    Nilai aktual sesi (score, errors, time, dst.) TIDAK diubah agar dashboard
    tetap menampilkan performa nyata yang terjadi.
    ML-profile ditambahkan sebagai field tambahan 'ml_profile' untuk dipakai
    di bar skill (Kecepatan, Akurasi, Konsistensi, Kemandirian).
    """
...
class AchievementPopup(tk.Frame):
    def __init__(self, master, badges, on_done, initial_idx=0,
        ...
        self._pre_blur_photo = pre_blur_photo   # screenshot yg diambil sebelum clear
        self.place(relx=0, rely=0, relwidth=1, relheight=1)
        self.lift()
        # Turunkan icon pojok agar tidak menembus popup
        _corner_icons_lower()
        self._bind_skip()
        self._show_next()
    ...
    def _bind_skip(self):
        ...
        try:
            ...
            # <Escape> sengaja TIDAK di-bind — ESC tidak boleh berfungsi
            # saat achievement popup tampil agar tidak bisa dilewati paksa.
        except Exception:
            pass
    ...
    def _show_next(self):
        ...
        # ── Mainkan SFX achievement ───────────────────────────────────────────
        _play_sfx(_SFX_ACHIEVEMENT)

        # ── Blur background ───────────────────────────────────────────────────
        # Gunakan pre_blur_photo (diambil sebelum screen di-clear) agar tidak hitam
        try:
            _root_ref = self.winfo_toplevel()
        except Exception:
            _root_ref = None
        ...
        blur_lbl = _place_blur_canvas(
            ...
            pre_captured=self._pre_blur_photo   # pass foto pre-capture
        )

        # Klik di area blur (di luar card) → skip badge
        blur_lbl.bind("<Button-1>", self._on_skip)

        # ── Outer glow ring (frame sedikit lebih besar, warna badge) ─────────
    ...
    def _finish(self):
        self._unbind_skip()
        _corner_icons_restore()   # kembalikan icon pojok ke atas
        try:
...
def _save_score_card(username, session, player_type, type_color, ai_message,
                     ...
                     parent_widget=None):
    """Score Card PNG — vibrant poster untuk pameran & sosmed.
    Ukuran 860x auto-height agar achievement section dinamis.
    Tidak ada blank space, tidak ada _PilImage.blend yang menggelapkan.
    """
    if not PIL_AVAILABLE:
        if parent_widget:
    ...
    try:
        import math as _math, random as _rnd, datetime as _dt
        ...
        # ── Canvas RGBA (gunakan RGBA agar polygon transparan tidak blend seluruh img) ──
        img  = _PilImage.new("RGBA", (W,H), BG+(255,))
        draw = _PilDraw.Draw(img)

        def _fill(c):   return c+(255,)   # RGB → RGBA opaque
        def _fillA(c,a):return c+(a,)     # RGB → RGBA dengan alpha
        ...
        def _tile(x,y,w,h,icon,lbl,val,col):
            ...
            # Truncate val jika terlalu panjang
            val_str = str(val)[:8]
        ...
        # Radar polygon — RGBA layer terpisah, tidak merusak warna lain
        rpts = []
        ...
        def _donut(cx,cy,ro,ri,pct,col,label,val):
            mid_r = (ro + ri) / 2   # jari-jari tengah ring, untuk posisi dot

            # ── Track ring (background netral) ────────────────────────
            # Pieslice penuh → potong tengah → hasilnya ring netral
            draw.ellipse([(cx-ro,cy-ro),(cx+ro,cy+ro)], fill=_fill(BORDER2))
            draw.ellipse([(cx-ri,cy-ri),(cx+ri,cy+ri)], fill=_fill(CARD))

            # ── Arc aktif ─────────────────────────────────────────────
            if pct >= 100:
                # 100% → ring penuh berwarna (lingkaran penuh diperbolehkan)
                draw.ellipse([(cx-ro,cy-ro),(cx+ro,cy+ro)], fill=_fill(col))
                draw.ellipse([(cx-ri,cy-ri),(cx+ri,cy+ri)], fill=_fill(CARD))
            elif pct > 0:
                # Partial donut: pieslice → potong tengah
                ang_end = -90 + 360*pct/100
                draw.pieslice([(cx-ro,cy-ro),(cx+ro,cy+ro)],
                              start=-90, end=ang_end, fill=_fill(col))
                draw.ellipse([(cx-ri,cy-ri),(cx+ri,cy+ri)], fill=_fill(CARD))

                # Dot ujung arc — ditempatkan di tengah ketebalan ring
                ...
                # Cap awal arc (top, jam 12) agar terlihat rapi
        ...
        # ─────────────────────────────────────────────────────────────
        # SECTION: Visualisasi Mini Grid + Quick Stats (full width)
        # ─────────────────────────────────────────────────────────────
        ...
        # Warna sel — FILLED: gold, INITIAL: cyan/ACC (ditukar)
        COL_FILLED  = _blend(BG, GOLD, 0.72)   # gold kuat
        COL_INITIAL = _blend(BG, ACC,  0.78)   # biru/cyan kuat
        ...
        # ─────────────────────────────────────────────────────────────
        # SECTION: ACHIEVEMENT — Dinamis (auto-expand)
        # ─────────────────────────────────────────────────────────────
        ...
        # Resize canvas ke tinggi aktual
        ...
        # ── Logo box (bottom-right footer) ───────────────────────────────
        # Logo putih → butuh background gelap + border ACC agar kontras
        BOX_S   = 58                        # ukuran kotak logo (square)
        QX2     = W - BOX_S - 12           # 12px dari tepi kanan
        QY2     = FOOT_Y + (FOOT_H - 8 - BOX_S) // 2  # vertikal tengah footer
        _logo_path_sc = os.path.join(_ASSETS_DIR, "logo.png")
        _logo_pil_sc  = None
        try:
        ...
        if _logo_pil_sc is not None:
            ...
            # ── Paste logo: resize proporsional agar muat di dalam kotak ──
            _LP       = 11          # padding dalam kotak untuk logo
            _fit      = BOX_S - _LP * 2
            ...
            img.paste(_logo_rsz, (_lx, _ly), _logo_rsz)   # logo alpha = mask
            draw = _PilDraw.Draw(img)                       # refresh draw setelah paste

        else:
            # ── Fallback "AI" text jika logo.png tidak ditemukan ─────────
            draw.rounded_rectangle(
                [(QX2-2, QY2-2), (QX2+BOX_S+2, QY2+BOX_S+2)],
        ...
        # ── Simpan ───────────────────────────────────────────────────
        # Convert RGBA → RGB untuk PNG tanpa alpha (lebih kecil)
...
# =====================================================
# Trigger: Klik logo/title 7x dalam 4 detik di LoginScreen
#
...
_EASTER_EGG_VIDEO = os.path.join(_ASSETS_DIR, "easter_egg.mp4")
_EASTER_EGG_AUDIO = os.path.join(_ASSETS_DIR, "easter_egg.mp3")
...
class EasterEggOverlay(tk.Toplevel):
    ...
    def __init__(self, master, on_close_callback=None):
        ...
        self._running   = True
        self._cap       = None
        self._photo_ref = None   # cegah GC PhotoImage
        ...
        # ── Jendela full-screen tanpa chrome ──────────────────────────
        ...
        # Pilih warna latar sesuai tema aktif
    ...
    def _start_audio(self):
        """Putar audio Easter Egg tanpa menggantikan track musik utama.

        Strategi:
        - Musik utama di-pause (posisi di-preserve oleh pygame).
        - Easter Egg audio diputar via pygame.mixer.Sound (channel SFX
          terpisah) sehingga channel music tidak disentuh sama sekali.
        - Saat _close(), cukup unpause() agar musik lanjut dari posisi semula.
        - Fallback ke load()+play() jika Sound gagal load mp3 (pygame lama).
        """
        if not PYGAME_AVAILABLE:
            return
        self._egg_sound_obj     = None   # Sound object jika berhasil
        self._music_was_replaced = False  # True jika terpaksa pakai music channel

        try:
            pygame.mixer.music.pause()   # Pause musik utama, posisi tetap tersimpan
        except Exception:
            pass

        if os.path.exists(_EASTER_EGG_AUDIO):
            # Coba load sebagai Sound (channel terpisah — musik utama aman)
            try:
                ...
                pass
            # Fallback: terpaksa pakai music channel (posisi hilang setelah close)
            try:
    ...
    def _start_content(self):
        ...
        if cv2_ok and pil_ok and os.path.exists(_EASTER_EGG_VIDEO):
            self._start_video()
        else:
    ...
    def _play_video_frame(self):
        ...
        if not ret:
            # Video selesai — tutup overlay otomatis
            self._close()
            return
        ...
        # Fit dengan mempertahankan rasio aspek
        scale = min(sw / w, sh / h)
    ...
    def _anim_tick(self):
        ...
        # ── Spawn partikel baru setiap 2 frame ───────────────────────
        if t % 2 == 0:
            # Ledakan dari 3 titik acak secara bergantian
            origins = [
                (cx, cy),
    ...
    def _close(self, event=None):
        ...
        # Hentikan audio Easter Egg
        if PYGAME_AVAILABLE:
            # Jika easter egg audio pakai Sound object, hentikan lewat Sound
            if getattr(self, '_egg_sound_obj', None) is not None:
                ...
                # Fallback: audio ada di music channel, hentikan dari sana
                try:
                    pygame.mixer.music.stop()
                except Exception:
                    pass

        # Kembalikan musik utama.
        # - Jika Sound object dipakai (_music_was_replaced=False): channel musik
        #   utama masih intact → cukup unpause() agar lanjut dari posisi semula.
        # - Jika terpaksa pakai music channel (_music_was_replaced=True): track
        #   sudah terganti, satu-satunya pilihan adalah load()+play() dari awal.
        # Dalam kedua kasus, _music_paused dihormati: jika user sudah mematikan
        # musik sebelum Easter Egg dibuka, musik tetap mati setelah ditutup.
            ...
            try:
                ...
                if app._music_ready and not app._music_paused:
                    if not getattr(self, '_music_was_replaced', False):
                        # Music channel masih menyimpan posisi — lanjutkan saja
                        pygame.mixer.music.unpause()
                    else:
                        # Channel sudah diganti, harus reload dari awal
                        pygame.mixer.music.load(MUSIC_FILE)
                        pygame.mixer.music.set_volume(0.7)
...
def _trigger_easter_egg(root, on_close_cb=None):
    """Panggil dari mana saja untuk membuka EasterEggOverlay."""
    global _EASTER_EGG_ACTIVE
    if _EASTER_EGG_ACTIVE:
        return   # sudah aktif, jangan spawn ganda
    try:
...
def _patched_on_finish(self, session, ml):
    ...
    # Evaluasi achievement setelah sesi selesai
    data         = load_data()
    ...
    if earned_badges:
        # ── Ambil blur screenshot SEBELUM screen di-clear ─────────────────────
        # Jika diambil setelah _clear(), root sudah kosong/hitam → background hitam.
